...
exports.validateOrder = (req, res, next) => {
    const schema = Joi.object({
        ...
        recipentFirstName: Joi.string().min(2).max(50).when('deliveryToAnotherPerson', { is: true, then: Joi.required() }).label('Recipient First Name'),
        recipentLastName: Joi.string().min(2).max(50).when('deliveryToAnotherPerson', { is: true, then: Joi.required() }).label('Recipient Last Name'),
